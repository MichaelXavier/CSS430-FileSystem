class Test5 extends Thread {
  ...
  //SysLib.cout("checkpoint 1\n");
	fd = SysLib.open( "css430", "r" );
  //SysLib.cout("checkpoint 2\n");

	byte[] tmpBuf = new byte[16];
  //SysLib.cout("checkpoint 3\n");
	size = SysLib.read( fd, tmpBuf );
  //SysLib.cout("checkpoint 4\n");
	if ( size != 16 ) {
    //SysLib.cout("checkpoint 5\n");
	    SysLib.cout( "size = " + size + " (wrong)\n" );
	    SysLib.close( fd );
	    return false;
	}
  //SysLib.cout("checkpoint 6\n");
	for ( int i = 0; i < 16; i++ )
	    if ( tmpBuf[i] != buf16[i] ) {
